"""Evaluate all TMC-Net models from the Nt/s ablation study."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate all TMC-Net ablation models.")
...
def evaluate_single_model(config_name, checkpoint_path, args, snr_range):
    """Evaluate a single model and return results."""
    print(f"\nEvaluating {config_name}...", flush=True)
    device = torch.device(args.device)
    ...
    print(
        ...
        + f", sigma_e^2={config.csi_error_var:g}"
        + f", phase_bits={config.ris_phase_bits}",
        flush=True,
    )
...
def main():
    ...
    base_checkpoint_dir = ROOT / args.checkpoint_dir

    all_reports = []
    for param_config in PARAM_CONFIGS:
        config_name = param_config["name"]
        checkpoint_path = base_checkpoint_dir / config_name / "tmc_best.pt"

        if not checkpoint_path.exists():
            print(f"Warning: Checkpoint not found for {config_name}: {checkpoint_path}")
            continue

        report = evaluate_single_model(config_name, checkpoint_path, args, snr_range)
        all_reports.append(report)

        report_path = ROOT / "outputs" / "reports" / f"tmc_infer_{config_name}.json"
        save_json(report_path, report)
        print(f"  Saved: {report_path}")

    summary_path = ROOT / "outputs" / "reports" / "tmc_ablation_infer_summary.json"
    ...
    print(f"{'='*70}")
    print(f"\n{'Config':<12} {'n_t':>4} {'s':>4} {'Avg BER (TMC)':>14} {'Avg BER (Baseline)':>18}")
    print("-" * 70)
    for report in all_reports:
        avg_ber_tmc = np.mean(report["tmc_corrected"]["ber"])
        avg_ber_baseline = np.mean(report["practical_baseline"]["ber"])
        print(f"{report['config_name']:<12} {report['n_t']:>4} {report['s']:>4} {avg_ber_tmc:>14.5f} {avg_ber_baseline:>18.5f}")
    ...
    try:
        ...
        # Plot TMC-corrected (solid) and baseline (dashed) for each config
        for idx, report in enumerate(all_reports):
            snr = report["snr_db"]
            color = colors[idx % len(colors)]
            marker = markers[idx % len(markers)]
            label = f"{report['config_name']} (n_t={report['n_t']}, s={report['s']})"

            # TMC corrected (solid lines with markers)
            plt.semilogy(
                snr,
                np.maximum(report["tmc_corrected"]["ber"], 1e-7),
                color=color,
                linestyle="-",
                marker=marker,
                markevery=2,
            ...
            # Baseline (dashed lines with markers)
            plt.semilogy(
                snr,
                np.maximum(report["practical_baseline"]["ber"], 1e-7),
                color=color,
                linestyle="--",
                marker=marker,
                markevery=2,
        ...
        plt.title("TMC-Net Ablation: BER vs SNR for Different Nt/s Configurations", fontsize=14)
        plt.grid(True, which="both", linestyle="--", alpha=0.6)
        plt.legend(loc="best", fontsize=9, ncol=2)
        plt.tight_layout()

        figure_path = Path(args.figure_path) if args.figure_path else ROOT / "outputs" / "figures" / "tmc_ablation_ber.png"
